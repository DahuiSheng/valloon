...
import { Container, VStack, Input, Text, Box, HStack, Avatar, Button } from "@chakra-ui/react";
import Link from "next/link";
import { useState } from "react";

export default function Form() {
    // first name（苗字）を保存する関数
    const [firstname, setFirstName] = useState<string>("");
    // second name（名前）を保存する関数
    const [secondname, setSecondName] = useState<string>("");
    // メールアドレスを保存する関数
    const [mail, setMail] = useState<string>("");
  
    // 名前入力フォームのボタンの機能を使う関数
    ...
    const handleSubmit = () => {
      ...
      if (checkName(firstname)) {
        console.log("firstname （苗字）の入力がありません");
      } else {
      ...
      if (checkName(secondname)) {
        console.log("secondname（名前）の入力がありません");
      } else {
    ...
    // nameに値が含まれているか確認
    function checkName(name: string):string | void {
      if (name === "") {
        return "値が含まれていません";
      }
    }
    ...
    // mailに値が含まれているか確認
    function checkMail(mail: string):string | void {
      if (mail === "") {
        return "値が含まれていません";
      }
    }
